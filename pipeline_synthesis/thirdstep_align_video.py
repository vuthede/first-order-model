...
def pipelineneckclone(img_hd, img_syn, bbox):
    ...
    center = ( (xmin+xmax)//2 + paddinng_x_for_center, (ymin+ymax)//2 )
...
def get_main_opt():
    parser = argparse.ArgumentParser("Params for align video")
    parser.add_argument('--synthesisvideo', type=str, default="synthesisvideo.mp4")
    parser.add_argument('--fullhdvideo', type=str, default="fullhd_video.mp4")
    parser.add_argument('--outputvideo', type=str, default="aligned_video.mp4")
    parser.add_argument('--startframehd', type=int, default=2*30*60)
...
def demo_fullhd(main_args):



    out = cv2.VideoWriter(f'{main_args.outputvideo}', cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1920, 1080))
    out_images = f"{main_args.outputvideo}".replace(".mp4","")
    if not os.path.isdir(out_images):
        os.makedirs(out_images)
    

    # HD video and synthesis video
    cap = cv2.VideoCapture(f'{main_args.synthesisvideo}')
    cap_hd = cv2.VideoCapture(f'{main_args.fullhdvideo}')

    # Warm up:
    cap_hd.set(1, int(main_args.startframehd))
    ...
    while True:
        ...
        if i %(6*30) ==0: # Loop 5s
            cap_hd.set(1, int(main_args.startframehd)) # Loop fullhd

        if i%1==0:
            print(f"Aligninig frame {i}")
...
if __name__=="__main__":
    main_args = get_main_opt()
    print(main_args)
    demo_fullhd(main_args)
