...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1, output_video="crop_video.mp4"):
    ...
    return f'ffmpeg -i {inp} -ss {start} -t {time} -filter:v crop={w}:{h}:{left}:{top},scale={scale} {output_video}'
...
def compute_bbox_trajectories(trajectories, fps, frame_shape, args):
    ...
    for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
        if (end - start) > args.min_frames:
            command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=args.inp, image_shape=args.image_shape, increase_area=args.increase, output_video=args.outputcroppedvideo)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--outputcroppedvideo", type=str, default="crop_video.mp4")
