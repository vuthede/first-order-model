...
def inpaint_face_fancy(img, bbox):
    # Get face mask
    x1,y1,x2,y2 = map(int, bbox)
    mask = faceParsing.get_face_mask(img, bbox=(x1,y1,x2,y2), apply_dilation=True)
...
def inpaint_face_morefancy(img, bbox, line=600):
    # Get face mask
    x1,y1,x2,y2 = map(int, bbox)
    mask = faceParsing.get_face_mask_including_neck(img, bbox=(x1,y1,x2,y2), apply_dilation=True)
    mask[line:,:,:] = [0,0,0]
    ...
    result = cv2.resize(result, (img.shape[1], img.shape[0]))
    result = faceParsing.paint_part_again(img[:,:,::-1], result, bbox, 14)
...
def scale_image_by_neck_masks(neckmask_src, neckmask_tar, img_src):
    index = np.where(neckmask_src==255)[:2][::-1]
    xmin,ymin = np.min(index, axis=1)
    xmax,ymax = np.max(index, axis=1)
    ...
    index1 = np.where(neckmask_tar==255)[:2][::-1]
    xmin1,ymin1 = np.min(index1, axis=1)
    xmax1,ymax1 = np.max(index1, axis=1)
    ...
    ### -----------Method3############
    # src = np.float32([center, center-np.array(w,h), center+np.array(w,h) ])
    # des = np.float32([center1, center1-np.array(w1,h1), center1+np.array(w1,h1)])

    ### -----------Method3############


    ###------Method 1#########
    # src = np.float32([[center[0]-w/2, center[1]], [center[0]+w/2, center[1]], center])
    # des = np.float32([[center[0]-w1/2, center[1]], [center[0]+w1/2, center[1]], center1])
    # src = np.float32([[xmin,ymin], [xmax, ymin], center])
    # des = np.float32([[xmin,ymin], [xmax, ymin], center])
    ###------Method 1#########


    ###---------Method 2#########
    vl = np.array([xmin1, ymin1]) - center1
    vr = np.array([xmax1, ymin1]) - center1
    pleft = center + vl
    pright = center + vr
    src = np.float32([[xmin,ymin], [xmax, ymin], center])
    des = np.float32([pleft, pright, center])
    ###---------Method 2#########
    ...
    print("Loss balance ", np.linalg.norm(center1-center))

    M = cv2.getAffineTransform(src, des)
    print(M)
    img_des = cv2.warpAffine(img_src, M, (img_src.shape[1], img_src.shape[0]), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT_101)
    cv2.imshow("Synthesis original:", img_src)
    cv2.imshow("Synthesis original scale:", img_des)

    # cv2.imwrite("Synthesisdes.png", img_des)

    return img_des

def pipelineneckclonescale(img_hd, img_syn, bbox):
    # Inpaint face in Hd image
    # img_hd_face_inpainted = inpaint_face_including_neck(img_hd, bbox)
    # img_hd_face_inpainted = inpaint_face_fancy(img_hd, bbox)
    img_hd_face_inpainted = inpaint_face_morefancy(img_hd, bbox)
    ...
    crophd_neckmask = faceParsing.get_part_mask_by_id_512(crophd, 14).astype(np.uint8)
    warp2d = scale_image_by_neck_masks(warp2d_neckmask, crophd_neckmask, warp2d)
    ...
    ##--------------Dilate face. Combine face and neck. Clone them as an object
    kernel = np.ones((7,7),np.uint8)
    warp2d_facemask = cv2.dilate(warp2d_facemask,kernel,iterations = 3)
    # kernel = np.ones((5,5),np.uint8)
    # warp2d_neckmask = cv2.dilate(warp2d_neckmask,kernel,iterations = 3)
    warp2d_facemask = warp2d_facemask |  warp2d_neckmask
    ...
    ##--------------Clone neck and face as an object---------------------
    # kernel = np.ones((7,7),np.uint8)
    # warp2d_facemask = cv2.dilate(warp2d_facemask,kernel,iterations = 1)
    # index = np.where(warp2d_facemask==255)[:2][::-1]
    # xmin,ymin = np.min(index, axis=1)
    # xmax,ymax = np.max(index, axis=1)
    # center = ( (xmin+xmax)//2, (ymin+ymax)//2)
    ...
    ##-------------------------Clone neck and face separatly-------------------
    # kernel = np.ones((7,7),np.uint8)
    # warp2d_facemask = cv2.dilate(warp2d_facemask,kernel,iterations = 3)
    # index = np.where(warp2d_neckmask==255)[:2][::-1]
    # xmin,ymin = np.min(index, axis=1)
    # xmax,ymax = np.max(index, axis=1)
    # center = ( (xmin+xmax)//2, (ymin+ymax)//2)
    # final_image = cv2.seamlessClone(warp2d, crophd_inpainted, warp2d_neckmask, center, cv2.NORMAL_CLONE)
    # index = np.where(warp2d_facemask==255)[:2][::-1]
    # xmin,ymin = np.min(index, axis=1)
    # xmax,ymax = np.max(index, axis=1)
    # center = ( (xmin+xmax)//2, (ymin+ymax)//2)
...
def find_padding_x_for_center(neckmask_src, neckmask_tar):
    index = np.where(neckmask_src==255)[:2][::-1]
    xmin,ymin = np.min(index, axis=1)
    xmax,ymax = np.max(index, axis=1)
    center = np.array([(xmin+xmax)//2 , (ymin+ymax)//2])
...
def pipelineneckclone(img_hd, img_syn, bbox):
    ...
    warp2d_neckmask = faceParsing.get_part_mask_by_id_512(warp2d, 14).astype(np.uint8)
    crophd_neckmask = faceParsing.get_part_mask_by_id_512(crophd, 14).astype(np.uint8)
    paddinng_x_for_center = find_padding_x_for_center(warp2d_neckmask, crophd_neckmask)
    ...
    ####IMPROTTANT!!!! Add padding y for center
    center = ( (xmin+xmax)//2, (ymin+ymax)//2 )
